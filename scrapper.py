...
for row in df2.iterrows():  # Parse through each url in the list.
    try:
        try:
            req1 = Request(row[1]['URL'], headers={
...
with open('result_contact_JSON.json', 'w') as f:
    json.dump(rows, f, sort_keys=False, indent=4, separators=(',', ': '), encoding='utf-8')
"""
Nowlet's dive a little deeper and understand the code.
Step 1: Ask the user to input a keyword(s).Pass the user input to the function “search1
        " contained in the class Google.
user_input = raw_input("Enter your search string : ")
Google.search1(user_input)
Step2: Now search google for top 10 pages and extract all the URLs contained
       in “cite” tag, URL titles which are contained in “h3” tag, and description
       from “span” tag.Store all the extracted data in the lists “url_list”, “title_list” 
       and “description_list” respectively.
